...
# By definition, a set of tows (called a TowSet)
# is a PolySet of polylines with only two points,
# the start and end of each tow.
# A TowSet contains only PIDs, not SIDs, and has
# two POS values 1 and 2 for each tow.
#

# Function to generate a random set of tows
# within a given square region.
...
# Function to expand tows in a TowSet to rectangles
...
genTows <- function() {
  ...
  totArea <- s^2;
  meanL <- swpArea/(n*w);
  setWinVal(c(meanL=meanL,nPoly=nPoly,nHole=nHole,nVert=nVert,
    swpArea=swpArea,impArea=impArea,totArea=totArea));
  invisible(); };
...
plotTow <- function() {
  ...
  a <- PBS.win$action[1];
  if (a=="L") {
    plotMap(towLines,type="n",xlim=rng0,ylim=rng0);
    addLines(towLines); };
  if (a=="T")
    plotMap(towPolys,col="red",xlim=rng1,ylim=rng1);
  if ((a=="U") | (a=="C")) {
    myCol <-
      c("red","green","blue","yellow","orange","violet","cyan",
    ...
    if (a=="U")
      plotMap(towAll,polyProps=towProps,xlim=rng1,ylim=rng1,colHoles="white");
    if (a=="C") {
      #par(mfrow=c(1,2),mar=c(0.1,1.0,0.1,0.3),mgp=c(.2,.2,0));
      par(mfrow=c(2,1),mar=c(1.2,1.2,0.1,0.1),mgp=c(2,.1,0),las=1);
